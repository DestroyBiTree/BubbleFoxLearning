...
bin log（也叫做变更日志），他记录了数据库所有执行的DML和DDL，但是不包括没有任何修改的数据的语句（<font color='orange'>SELECT、SHOW</font>等）。

使用场景：

1. <font color='orange'>数据恢复</font>。如果数据库意外停止，可以根据二进制日志文件来查看用户执行了哪些操作，根据操作恢复数据库服务器。
2. <font color='orange'>数据复制</font>。用于主从复制的一致性。
...
<img src="C:\Users\SalmonRun\AppData\Roaming\Typora\typora-user-images\image-20230328170555241.png" alt="image-20230328170555241" style="zoom:67%;" />

使用**两阶段提交**后，写入binlog时发生异常也不会有影响。MySQL根据redo log日志恢复数据时，发现redo log处于prepare的阶段，并且没有对应的binlog日志，就会回滚该事务。

![image-20230328170642981](C:\Users\SalmonRun\AppData\Roaming\Typora\typora-user-images\image-20230328170642981.png)
